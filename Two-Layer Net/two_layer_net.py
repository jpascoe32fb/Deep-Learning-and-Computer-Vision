...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = torch.maximum(torch.tensor(0, dtype=(X.mm(W1) + b1).dtype, device=(X.mm(W1) + b1).device), (X.mm(W1) + b1))
...
def nn_forward_backward(
    ...
    data_loss = -torch.log(correct_probs).sum() / N

    reg_loss = 0.5 * reg * (torch.sum(W1 * W1) + torch.sum(W2 * W2))
    ...
    grads['W2'] = h1.t().mm(dscores) + reg * W2
    grads['b2'] = torch.sum(dscores, dim=0)

    dh1 = dscores.mm(W2.t())
    dh1[h1 <= 0] = 0  

    grads['W1'] = X.t().mm(dh1) + reg * W1
        ...
        for hidden_size in hidden_sizes:
            for reg in regularization_strengths:
                for lr_decay in learning_rate_decays:
                    net = TwoLayerNet(input_size=X_train.size(1), hidden_size=hidden_size, output_size=10)
