...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0
    for t in range(T):
      prev_h, cache1 = rnn_step_forward(x[:,t,:], prev_h, Wx, Wh, b)
      h[:, t, :] = prev_h
      cache.append(cache1)
    ##########################################################################
    #                             END OF YOUR CODE                           #
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
      dout = dprev_h + dh[:, t, :]
      dxm, dprev_h, dWxm, dWhm, dbm = rnn_step_backward(dout, cache.pop())
      dx[:,t,:] = dxm
      dWx += dWxm
      dWh += dWhm
      db += dbm
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = torch.nn.Conv2d(400, hidden_dim,(1,1))
        self.model_emb = WordEmbedding(vocab_size, wordvec_dim)
